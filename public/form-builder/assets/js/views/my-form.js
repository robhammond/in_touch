define([
      ...
      , "text!templates/app/renderformIframe.html"
], function(
  $, _, Backbone
  , TempSnippetView
  , PubSub
  ...
  return Backbone.View.extend({
    ...
    , initialize: function(){
      ...
      this.renderFormIframe = _.template(_renderFormIframe);
      this.render();
    }
    ...
    , render: function(){
      ...
      $("#renderIframe").val(that.renderFormIframe({
        multipart: this.collection.containsFileType(),
        text: _.map(this.collection.renderAllClean(), function(e){
          return e.html()
        }).join("\n")
      }));
      $('#publishForm').unbind('click').click(function(e) {
        var html = $("#renderIframe").val();
        // console.log('sending', html);
        $.ajax({
            url:"http://localhost:3000/save-form",
            type: "POST",
            data: {form : html},
            crossDomain: true,
            dataType: 'text',
            success:function(response){
              // console.log('You created your form sucessfully', response);
            },
            ...
            complete: function (response) {
              ...
              if (response.responseText) {
                $("#result").text('<iframe src="'+response.responseText+'"></iframe>');
              }
            }
        });
      });
